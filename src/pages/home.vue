  ...
  <f7-page name="home">
    <template v-if="isLoggedIn">
      ...
      <f7-navbar large>
        ...
        <f7-nav-right>
          <f7-link icon="f7-icons icon-header-alarm" href="/notifications/"></f7-link>
        </f7-nav-right>
      </f7-navbar>
      ...
      <template v-if="currentTrip && currentTrip.isTripStarted">
        ...
        <f7-toolbar bottom no-shadow class="custom-toolbar">
          <div class="row width-100 padding-horizontal">
            <f7-button color="red" fill class="col-100 text-uppercase" @click="endTrip">{{$ml.get('COM_MSG005')}}</f7-button>
          </div>
        </f7-toolbar>
        ...
        <f7-block>
          <f7-link :href="`/accident-report/?imei=${ selectedAsset }`" icon="f7-icons icon-notification size-18 margin-right-half" :text="$ml.get('HOME_MSG013')" color="orange" class="button button-fill vertical-align-middle"></f7-link>
        </f7-block>
      ...
      <template v-else>
        ...
        <f7-list>
          ...
          >
            ...
            <select name="checkLists" v-model="selectedCheckList" required validate>
              <option
  ...
  export default {
    data() {
      ...
      return {
        ...
        isImmobilised: localStorage.IMMOBILISATION_STATE && localStorage.IMMOBILISATION_STATE === 'on',
    ...
    watch: {
      ...
      async isLoggedIn(val) {
        if (val) {
          ...
          if (!this.$f7.methods.isObjEmpty(additionalFlags) && additionalFlags.isTripStarted && !this.$f7.methods.isObjEmpty(additionalFlags.Trip)) {
            let obj = {
      ...
      currentTrip(val){
        ...
        if(val && val.isTripStarted){
          ...
          this.isImmobilisationSupported = (asset.Permissions & enumAssetPermissions.immobilisation) > 0
          this.getCurrentTripDuration()
          this.currentTripDurationTimer = setInterval(this.getCurrentTripDuration,60*1000)
        }else{
    ...
    //console.log($$('.status_page').length);
      ...
      getCurrentTripDuration(){
        let dateDifference = getDifferenceBTtwoDates(moment(this.currentTrip.Trip.StartTime, tFormat[0]), moment());
        this.currentTripDuration = moment.duration(dateDifference, "milliseconds").format('d[d] h[h] m[m]');
      }
    },
    created() {
