...
<script>
  ...
  export default {
          ...
          methods: {
            ...
            logout(){
              ...
              if(!this.methods.isObjEmpty(notifications)){
                this.methods.setInStorage({ name: 'notifications', data: notifications });
              }
            ...
            getFromStorage: function(name){
              ...
              if (name) {
                switch (name){
                  ...
                  case 'notifications':
                    str = localStorage.getItem("COM.QUIKTRAK.PRESTART.NOTIFICATIONS");
                    if(str) {
                      ret = JSON.parse(str);
                    }else{
                      ret = {};
            ...
            setInStorage: function(params){
              if (typeof(params) == 'object' && params.name && params.data) {
                switch (params.name){
                  ...
                  case 'notifications':
                    localStorage.setItem("COM.QUIKTRAK.PRESTART.NOTIFICATIONS", JSON.stringify(params.data));
                    break;
            ...
            setNotificationList: function(list){
              ...
              self.methods.setInStorage({name: 'notifications', data: pushList})
            },
            getNewNotifications(){
              return new Promise(async (resolve, reject) => {
                let newNotifications = await store.dispatch('GET_NEW_NOTIFICATIONS', {
                ...
                if(!newNotifications || !Array.isArray(newNotifications) || !newNotifications.length){
                  resolve([])
                  return
                }
    ...
    watch: {
      error(err) {
        this.$f7.dialog.alert(this.$ml.get('PROMPT_MSG001'), this.$ml.get('PROMPT_MSG000'))
      ...
      apiValidationError(err) {
        ...
        switch (err.method) {
          ...
          case 'start_trip':
            if(err.MajorCode === '100' && err.MinorCode === '1002' && err.Data === 'ANOTHER_TRIP_IS_RUNNING'){
              this.$f7.methods.customDialog({
                title: this.$ml.get('PROMPT_MSG000'),
                text: this.$ml.get('PROMPT_MSG014')
              })
            }else if(err.MajorCode === '100' && err.MinorCode === '1002' && err.Data === 'NO_POSITION'){
              this.$f7.methods.customDialog({
                title: this.$ml.get('PROMPT_MSG000'),
                text: this.$ml.get('PROMPT_MSG017')
              })
            }else if(err.MajorCode === '100' && err.MinorCode === 'ASSET_BE_USEING'){
              this.$f7.methods.customDialog({
                title: this.$ml.get('PROMPT_MSG000'),
                text: this.$ml.with('name', err.Data.FirstName + ' ' + err.Data.SurName).get('PROMPT_MSG031')
              })
            }else if(err.MajorCode === '100' && err.MinorCode === 'DRIVER_BIND_OTHER_CAR'){
              this.$f7.methods.customDialog({
                title: this.$ml.get('PROMPT_MSG000'),
...
//{"MajorCode":"100","MinorCode":"ASSET_BE_USEING","Data":{"FirstName":"driver","SurName":"driver","Mail":"prestart.driver@gmail.com","Code":"0d1aec9f-d5ac-4668-99c6-6bf85758b7e8"}}
          case 'end_trip':
            if(err.MajorCode === '100' && err.MinorCode === '1002' && err.Data === 'NO_TRIP_RECORD'){
              let obj = {
                isTripStarted: false,
                Trip: {}
              };
