  ...
  >
    <f7-page>
      ...
      <div class="playback-wrapper">
        ...
        <l-marker  v-if="marker.length" :icon="markerIcon" :lat-lng="marker" ></l-marker>
...
<script>
  ...
  export default {
   ...
   async   addMarker(event) {
      ...
      this.address = await this.$store.dispatch('fetchAddress', {lat: event.latlng.lat, lng: event.latlng.lng});
